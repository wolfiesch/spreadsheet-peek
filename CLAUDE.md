...
- **`.mcp.json`** - local MCP server declaration shared by Claude Code and Codex-style plugin installs. It should keep pointing at `mcp-app/bin/run-server.sh`, which resolves plugin-root environment variables before falling back to the repository-relative path.
