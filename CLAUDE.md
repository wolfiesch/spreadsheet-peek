...
- **`.github/workflows/benchmark.yml`** - runs `measure_tokens.py` on PR branch and master, posts (or updates) a single drift comment if output differs. `WOLFXL_CLI_VERSION` is pinned in the workflow env (currently `0.7.0`) and read from each checkout so CLI bump PRs compare the proposed PR binary against the current master baseline.
...
The current version contract is: `spreadsheet-peek 2.x` requires `wolfxl-cli >= 0.7.0` (the first release with the full `peek` / `map` / `agent` / `schema` subcommand surface). Current stable direct reads are `.xlsx` and `.xlsm`; broader format support and richer number-format rendering belong upstream in `wolfxl-cli`.
