...
- **`examples/generate_sample.py`** / **`examples/generate_wide_table.py`** / **`examples/generate_tall_ledger.py`** / **`examples/generate_messy_csv.py`** - deterministic sample generators. Commit the regenerated files alongside generator changes; the benchmark and delimited-format docs pin to them.
...
The current version contract is: `spreadsheet-peek 2.1.x` requires `wolfxl-cli >= 0.8.0` (the first release with the full `peek` / `map` / `agent` / `schema` surface plus direct `.xls`, `.xlsb`, `.ods`, `.csv`, `.tsv`, and `.txt` reads and common number-format-aware rendering).
